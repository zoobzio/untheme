...
/**
 * A raw Carbon scale object from `@carbon/colors`, e.g. `{ 10: "#edf5ff", …,
 * 100: "#001141" }`.
 */
 ...
 * }`)
 * that is mode-independent — the same hex serves both light and dark themes,
 * so
 * only one set of `{name}-{grade}` references is emitted per family. Light/dark
 * divergence happens in the system tokens, which alias different grades per
 * mode.
 *
 * @param name - Prefix for the token keys (e.g. "blue", "cool-gray").
 * @param scale - The family object from `@carbon/colors` (e.g. `blue`,
 * `coolGray`).
